...
-- 显式加载 libiconv（路径可能因安装方式不同而变化）
local libiconv = ffi.load("iconv")  -- 默认路径
-- 或 Homebrew 安装的路径
-- local libiconv = ffi.load("/usr/local/opt/libiconv/lib/libiconv.dylib")
...
local function iconv(s)
    ...
    local cd = libiconv.iconv_open('UTF-8', 'GBK')
    if cd == ffi_cast('iconv_t', ffi_new('int', -1)) then
        libiconv.iconv_close(cd)
        log(ERR, 'iconv_open error')
        return ''
    end

    cd = ffi_gc(cd, libiconv.iconv_close)
    ...
    local ok = libiconv.iconv(cd, src_buff, src_len, dst_buff, dst_len)
    if ok < 0 then
        libiconv.iconv_close(ffi_gc(cd, nil))
...
local function ip2long(ip)
    ...
    if libiconv.inet_aton(ip, inp) ~= 0 then
        return tonumber(libiconv.ntohl(inp[0].s_addr))
    end
